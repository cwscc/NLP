...
'''连接数据库，读取数据'''
conn = pymysql.connect(host="127.0.0.1", user="root", passwd="123456", db="crawl", charset='utf8')
sql1 = "select comment from hpscoreneg"
sql2 = "select comment from hpscorepos"
sql3 = "select comment from lenovoscoreneg"
sql4 = "select comment from lenovoscorepos"
...
data1.to_sql(name='hpscoreneg', con=engine_loacl, if_exists="append", index=False)
data2.to_sql(name='hpscorepos', con=engine_loacl, if_exists="append", index=False)
data3.to_sql(name='lenovoscoreneg', con=engine_loacl, if_exists="append", index=False)
data4.to_sql(name='lenovoscorepos', con=engine_loacl, if_exists="append", index=False)
