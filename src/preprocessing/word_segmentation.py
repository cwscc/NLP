...
def create_table():
    ...
    cursor.execute("DROP TABLE IF EXISTS poscomment")
    cursor.execute("DROP TABLE IF EXISTS negcomment")

    sql1 = """CREATE TABLE poscomment (
             `id` INT NOT NULL AUTO_INCREMENT,
             `comment` VARCHAR(5000) NOT NULL,
              PRIMARY KEY (`id`))"""
    sql2 = """CREATE TABLE negcomment (
             `id` INT NOT NULL AUTO_INCREMENT,
             `comment` VARCHAR(5000) NOT NULL,
              PRIMARY KEY (`id`))"""
...
if __name__ == '__main__':
    ...
    select_sql1 = "select `comment` from negcomment"
    insert_sql1 = "insert into `negcomment` (`comment`) values(%s)"
    
    select_sql2 = "select `comment` from poscomment"
    insert_sql2 = "insert into `poscomment` (`comment`) values(%s)"
