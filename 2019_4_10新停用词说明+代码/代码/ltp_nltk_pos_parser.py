...
if __name__ == "__main__":
    db_name = "fragmentExtraction"
    user = "root"
    password = "Xg123456"

    select_table1 = "hpscoreneg_punc"
    select_table2 = "hpscorepos_punc"
    select_table3 = "lenovoscoreneg_punc"
    select_table4 = "lenovoscorepos_punc"

    insert_table1 = "hpscoreneg_punc_pos"
    insert_table2 = "hpscorepos_punc_pos"
    insert_table3 = "lenovoscoreneg_punc_pos"
    insert_table4 = "lenovoscorepos_punc_pos"


    db = connect_db(db_name=db_name, user=user, password=password)
    create_table(db=db, table=insert_table1)
    create_table(db=db, table=insert_table2)
    create_table(db=db, table=insert_table3)
    create_table(db=db, table=insert_table4)


    no_process_data1 = data_processing(db=db,is_Nltk=False, table=select_table1)
    no_process_data2 = data_processing(db=db,is_Nltk=False, table=select_table2)
    no_process_data3 = data_processing(db=db,is_Nltk=False, table=select_table3)
    no_process_data4 = data_processing(db=db,is_Nltk=False, table=select_table4)

    pos_table1, _ = postagger_ltp(no_process_data1)
    pos_table2, _ = postagger_ltp(no_process_data2)
    pos_table3, _ = postagger_ltp(no_process_data3)
    pos_table4, _ = postagger_ltp(no_process_data4)

    insert_table(db=db, table=insert_table1,sentence_list=pos_table1)
    insert_table(db=db, table=insert_table2,sentence_list=pos_table2)
    insert_table(db=db, table=insert_table3,sentence_list=pos_table3)
    insert_table(db=db, table=insert_table4,sentence_list=pos_table4)
